# Transaction Analyzer

## Содержание 📚

- [О проекте](#о-проекте)
- [Статус проекта](#статус-проекта)
- [Стек технологий](#стек-технологий)
- [Структура проекта](#структура-проекта)
- [Использование](#использование)
- [Основной функционал](#основной-функционал)
- [Зависимости](#зависимости)
- [Тестирование](#тестирование)
- [Автор](#автор)
- [Лицензия](#лицензия)

## О проекте
**Transaction Analyzer** —учебный проект на Python для анализа банковских транзакций из Excel-файлов. Проект демонстрирует работу с данными, маскирование конфиденциальной информации, фильтрацию, сортировку и формирование отчетов.

**Основные возможности:**
- Маскирование номеров карт и счетов
- Фильтрация и сортировка операций
- Работа с JSON / CSV / Excel
- Генераторы и декораторы
- Вывод текущей стоимости валют и акций через API
- Тестирование с помощью pytest

## Статус проекта 
✅ Завершен 

🎓 Учебный проект

## Стек технологий
- Python 3.10+
- Poetry
- Pytest
- Pandas
- PostgreSQL
- JSON / CSV / Excel
- REST API

## Структура проекта
src/
  
    ├─ utils/               
    ├─ views/               
    ├─ services/            
    ├─ reports/             
    └─ main.py  
 
tests/


## Использование
1. Клонирование репозитория: 
```bash
git clone https://github.com/kshaab/Transaction-Analyzer
cd crswrk_1
```
2. Установка зависимостей и активация виртуального окружения:
```bash
poetry install
poetry shell
```
3. Запуск файла: 
```bash 
python src/main.py
```

## Основной функционал
 
### 🔸 Utils

`read_excel(excel_file)` 

Читает транзакции из Excel.

```python
>>> read_excel(excel_data)
```

`read_json(json_file)` 

Читает файл с настройками пользователя.

```python
>>> read_json(json_data)
```

`filter_by_date(df, date)`

Фильтрует данные по дате.

```python
>>> filter_by_date(df, date)
```

`view_cards_info(df, date)`

Выводит информацию по картам.

```python
>>> view_cards_info(read_data, required_date)
{'last_digits': '4556', 'total_spent': 703.0, 'cashback': 7}
```

`search_top(df, date)`

Собирает топ-5 операций по сумме. 

```python
>>> search_top(read_data, required_date)
{"date": "2020-11-02", "amount": 703.0, "category": "Аптеки", "description": "Аптека Вита"}, {"date": "2020-11-05", "amount": 462.0, "category": "Супермаркеты", "description": "WILDBERRIES"} и т.д.
```

`view_exchange_rate(settings)`

Текущий курс валют.

```python
>>> view_exchange_rate(read)
[{'currency': 'USD', 'rate': 79.38}, {'currency': 'EUR', 'rate': 93.25}]
```

`view_stock_prices(settings)`
Стоимость акций.

```python
>>> view_stock_prices(read)
{"stock": "AAPL", "price": 213.88}, {"stock": "AMZN", "price": 231.44}
```

### 🔸 Views

`greeting` 

Отображает приветствие по времени суток. 

```python
current_time = 13:00:00 
>>> greeting()
"Добрый день"
```

`view_homepage` 

Формирует JSON-ответ для домашней страницы.

```python
>>> view_homepage(input_date)
```

### 🔸 Services

`get_transactions(month: str, excel_file)` 

Сбор транзакций из Excel.

```python
>>> get_transactions("2021-11", excel_data)
```

`investment_bank(month: str, transactions, limit)` 

Сумма, отложенная в Инвесткопилку. 

```python
>>> investment_bank("2021-11", transactions, 10)
653.71
```

`find_phone_transactions(excel_file)`

Поиск транзакций с телефонами.

```python
>>> find_phone_transactions(excel_data)
{"transactions": [{"date": "19.11.2021", "amount": 200.0, "description": "Тинькофф Мобайл +7 995 555-55-55"}, {"date": "19.11.2021", "amount": 200.0, "description": "Тинькофф Мобайл +7 995 555-55-55"} и т.д.
```

### 🔸 Reports

`save_report_to_file(file = None)`

Декоратор для сохранения отчета.

`spending_by_category(t, category, date = None)`

Траты по категории за последние 3 месяца.

```python
>>> spending_by_category(transactions, "Транспорт", "2019-11-16")
{"категория": "Транспорт", "с": "2019-08-18", "по": "2019-11-16", "трат всего": 7795.43, "транзакций": 49}
```

### 🔸 Main

`main()`

Запускает основной сценарий работы программы.

## Зависимости

Управление зависимостями осуществляется через Poetry.
Файл конфигурации: `pyproject.toml` 


## Тестирование

В проекте используется фреймворк pytest.

Запуск тестов:
```bash
pytest tests/ -v
```

## Автор
[Ксения](https://github.com/kshaab)

## Лицензия
Проект распространяется в учебных целях.
